function estValide(button) {
  ...
  function main() {
    ...
    afficheur.sendMessage(
      "<p>Le jeu peut commencer ! <br /> Joueur " +
        joueurs[tour] +
        " c'est votre tour.</p>"
    );
    for (var i = 0, len = pions.length; i < len; i++) {
        ...
        if (!estValide(this)) {
          ...
          if (jeuEstFini) {
            afficheur.sendMessage(
              "<p>Le joueur " +
                joueurs[tour] +
                ' a gagné ! </p> <a href="index.html">Rejouer</a>'
            );
            return;
          }
          ...
          if (matchNul(pions)) {
            afficheur.sendMessage(
              '<p> Match Nul ! </p><a href="index.html">Rejouer</a>'
            );
            return;
          }
          ...
          tour++;
          tour = tour % 2;
          afficheur.sendMessage("<p>Joueur " + joueurs[tour] + " c'est à vous !</p>");
        }
      });
    }
  }
